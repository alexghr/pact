...
9. **Launcher.** The `pact` binary built from `cmd/pact` is the sole supported
   ...
   files as forced downloads or inline raw-text views. Raw-text views force
   `text/plain` with `nosniff` and a sandbox content security policy so
   agent-authored HTML is not executed in Pact's origin.
